...
const handleSubmit = async () => {
  ...
  try {
    ...
    success(
      response.data.message || 'Si el correo está registrado, recibirás un enlace de recuperación.',
      'Solicitud Enviada'
    )
    email.value = ''
  } catch (err) {
    const error = err as { response?: { data?: { message?: string } } }
    showError(error.response?.data?.message || 'Error al enviar el enlace de recuperación', 'Error')
  } finally {
    loading.value = false
  }
}
